 ...
 * FFM
 *
 * 0:バイオマス (Biomass) 4:バイオエタノール (Biofuel)
 */
public class BlockDrinkServerFFM extends BlockDrinkServerAbstract {
	// 素材アイテム
	public static Item[] itemType;

	public BlockDrinkServerFFM(int blockID, int textureStartCoord, Item[] itemType) {
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int par6, float par7, float par8, float par9) {
		...
		// バイオマス
		if (metaNum == 0) {
			entityplayer.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 400, 1)); // どんな効果が適切じゃろうか
			return true;
		}

		// バイオエタノール
		if (metaNum == 1) {
			entityplayer.addPotionEffect(new PotionEffect(Potion.blindness.id, 400, 1)); // それメタノールや
			return true;
		}
